...
public class FLaporanRugiLabaTahunan {

	JFrame frame;
	static FLaporanRugiLabaTahunan window;
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					window = new FLaporanRugiLabaTahunan();
	 ...
	 */
	public FLaporanRugiLabaTahunan() {
		initialize();
	}
	...
	private void initialize() {
		...
		frame.setBounds(100, 100, 289, 115);
		...
		cmbTahun.setBounds(89, 6, 175, 26);
		...
		btnCetak.setBounds(175, 39, 89, 26);
	...
	private int AmbilTotalReturn(String tgl) {
		...
		try {
			String query = "select (tb_detail_return_penjualan.harga - tb_detail_return_penjualan.harga_beli) as modal, "
					...
					+ "where date_format(tb_return_penjualan.tanggal, '%Y') = ?";
			PreparedStatement ps = db.con.prepareStatement(query);
			ps.setString(1, tgl);
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
	...
	private void Cetak() {
		...
		long labaKotor = (AmbilTotalRugiLaba() - AmbilTotalPiutang());
		...
		try {
			...
			String reportName = ".\\laporan\\LaporanRugiLabaTahunan.jasper";
			...
			WindowListener exitListener = new WindowAdapter() {
				@Override
				public void windowClosing(WindowEvent e){
					FLaporanRugiLabaTahunan.window.frame.setEnabled(true);
					jf.dispose();
				}
			};
