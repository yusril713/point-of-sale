...
public class FLaporanRugiLabaHarian {

	JFrame frame;
	static FLaporanRugiLabaHarian window;
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					window = new FLaporanRugiLabaHarian();
	...
	private void initialize() {
		...
		cmbTahun.setBounds(80, 6, 191, 26);
		...
		cmbBulan.setBounds(80, 31, 191, 26);
		frame.getContentPane().add(cmbBulan);
		
		cmbTanggal = new JComboBox<String>();
		cmbTanggal.setBounds(80, 56, 191, 26);
		...
		btnCetak.setBounds(182, 89, 89, 26);
	...
	private void TampilTahun() {
		...
		try {
			...
			String query = "select a.tahun from "
					+ "((select distinct date_format(tanggal, '%Y') as tahun from tb_transaksi) "
					+ "union "
					+ "(select distinct date_format(tanggal, '%Y') as tahun from tb_transaksi_kredit)) as a "
	...
	private void TampilTanggal() {
		...
		try {
			...
			String query = "select a.tanggal from "
					+ "((select distinct date_format(tanggal, '%d') as tanggal "
					+ "from tb_transaksi where date_format(tanggal, '%Y') = ? "
					...
					+ "from tb_transaksi_kredit where date_format(tanggal, '%Y') = ? "
					+ "and date_format(tanggal, '%m') = ? "
					+ "order by cast(tanggal as unsigned) desc)) as a "
					+ "order by a.tanggal desc";
			PreparedStatement ps = db.con.prepareStatement(query);
			ps.setString(1, cmbTahun.getSelectedItem().toString());
			ps.setString(2, cmbBulan.getSelectedItem().toString());
			ps.setString(3, cmbTahun.getSelectedItem().toString());
			ps.setString(4, cmbBulan.getSelectedItem().toString());
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
	...
	private long AmbilTotalPiutang() {
		...
		try {
			String query = "select sum(sisa_hutang)as tot from tb_hutang "
					...
					+ "where date_format(tb_transaksi_kredit.tanggal, '%d%m%Y') = ?";
			PreparedStatement ps = db.con.prepareStatement(query);
			ps.setString(1, cmbTanggal.getSelectedItem().toString() 
	...
	private long AmbilTotalRugiLaba() {
		...
		try {
			...
			ps.setString(1, cmbTanggal.getSelectedItem().toString() 
					+ cmbBulan.getSelectedItem().toString() 
					+ cmbTahun.getSelectedItem().toString());
			ps.setString(2, cmbTanggal.getSelectedItem().toString() 
					+ cmbBulan.getSelectedItem().toString() 
					+ cmbTahun.getSelectedItem().toString());
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
	...
	private int AmbilTotalReturn(String tgl) {
		...
		try {
			String query = "select (tb_detail_return_penjualan.harga - tb_detail_return_penjualan.harga_beli) as modal, "
					...
					+ "where date_format(tb_return_penjualan.tanggal, '%d%m%Y') = ?";
			PreparedStatement ps = db.con.prepareStatement(query);
			ps.setString(1, tgl);
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
	...
	private void Cetak() {
		...
		try {
			...
			String reportName = ".\\laporan\\LaporanRugiLabaHarian.jasper";
			...
			WindowListener exitListener = new WindowAdapter() {
				@Override
				public void windowClosing(WindowEvent e){
					FLaporanRugiLabaHarian.window.frame.setEnabled(true);
					jf.dispose();
				}
			};
